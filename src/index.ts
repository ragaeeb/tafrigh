...
import { cleanupGeneratedFiles, createTempDir } from './utils/io.js';
...
const transcribe = async (file: string, options: TranscribeFilesOptions): Promise<string> => {
    ...
    logger.info(`transcribe ${file} using ${JSON.stringify(options)}`);
    ...
    if (chunkFiles.length > 0) {
        ...
        outputFile = await writeTranscripts(
            transcripts,
            options.outputOptions || { format: OutputFormat.Json, outputDir, filename: path.parse(file).name },
        );
    ...
    if (!options.preventCleanup) {
        await cleanupGeneratedFiles(chunkFiles, filePath);
    }
